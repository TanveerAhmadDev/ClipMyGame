...
const MobileOpportunityFilters = ({
  filterOptions = {},
  filters = {},
  onUpdateFilter,
  onReset,
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="block md:hidden col-span-full w-full overflow-hidden "
          >
            <div className=" md:hidden flex gap-2 mt-2 pb-2 overflow-x-auto overflow-y-hidden scrollbar-thin [&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full ">
              {filteredItems.map((item) => {
                ...
                return (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.96 }}
                    className={` md:hidden shrink-0 inline-flex items-center gap-2 px-4 py-2 rounded-full border text-sm font-medium whitespace-nowrap ${isActive ? "bg-zinc-500 border-zinc-500 text-white" : "bg-white border-gray-200 text-gray-700 dark:bg-zinc-900 dark:border-zinc-700 dark:text-gray-300"} `}
                  >
...
export default MobileOpportunityFilters;
